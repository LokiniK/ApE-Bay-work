		...
		if(href_list["pen"])
			...
			if(P && (P.loc == src) && istype(P, /obj/item/paper) )
				...
				if(!(istype(usr, /mob/living/carbon/human) || isghost(usr) || istype(usr, /mob/living/silicon)))
					show_browser(usr, "<HTML><HEAD><TITLE>[P.name]</TITLE></HEAD><body><meta charset='utf-8'>[stars(P.info)][P.stamps]</BODY></HTML>", "window=[P.name]")
					onclose(usr, "[P.name]")
				else
					show_browser(usr, "<HTML><HEAD><TITLE>[P.name]</TITLE></HEAD><body><meta charset='utf-8'>[P.info][P.stamps]</BODY></HTML>", "window=[P.name]")
					onclose(usr, "[P.name]")
